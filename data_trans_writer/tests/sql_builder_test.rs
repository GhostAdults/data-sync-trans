...
fn test_build_base_sql_insert_postgres() {
    ...
    let sql = build_base_sql("test_table", &columns, &keys, &nonkeys, WriteMode::Insert, DbKind::Postgres).unwrap();
...
fn test_build_base_sql_insert_mysql() {
    ...
    let keys = vec!["id".to_string()];
    let nonkeys = vec!["name".to_string()];
    let sql = build_base_sql("test_table", &columns, &keys, &nonkeys, WriteMode::Insert, DbKind::Mysql).unwrap();
    assert_eq!(sql, "INSERT INTO test_table (id, name)");
}
...
fn test_build_base_sql_upsert_postgres() {
    ...
    let sql = build_base_sql("test_table", &columns, &keys, &nonkeys, WriteMode::Upsert, DbKind::Postgres).unwrap();
...
fn test_build_base_sql_upsert_mysql() {
    ...
    let keys = vec!["id".to_string()];
    let nonkeys = vec!["name".to_string(), "value".to_string()];
    let sql = build_base_sql("test_table", &columns, &keys, &nonkeys, WriteMode::Upsert, DbKind::Mysql).unwrap();
    assert!(sql.contains("INSERT INTO test_table (id, name, value)"));
...
fn test_build_base_sql_upsert_postgres_no_keys_error() {
    ...
    let nonkeys = vec!["name".to_string(), "value".to_string()];
    let result = build_base_sql("test_table", &columns, &keys, &nonkeys, WriteMode::Upsert, DbKind::Postgres);
